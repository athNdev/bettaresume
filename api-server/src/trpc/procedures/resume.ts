...
export const resumeRouter = router({
  ...
  list: protectedProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      // Fetch only resume metadata without sections for faster dashboard loading
      ...
      // Parse JSON fields - sections removed for performance
  ...
  duplicate: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      // Duplicate sections - batch insert for performance
      if (original.sections.length > 0) {
        ...
        await ctx.db.insert(sections).values(newSections);
      }
