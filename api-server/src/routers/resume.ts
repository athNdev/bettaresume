...
import { ownedResumeProcedure } from "../procedures";
import type { Resume, ResumeMetadata } from "@bettaresume/types";
...
// Helper to serialize tags/metadata for SQLite
...
const parseMetadata = (metadata: string | null): ResumeMetadata | null => {
  if (!metadata) return null;
  try {
    return JSON.parse(metadata) as ResumeMetadata;
  } catch {
    return null;
  }
};
...
export const resumeRouter = createTRPCRouter({
  ...
  // Get resumes by user ID (for public profiles)
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (original.sections.length > 0) {
        await ctx.db.section.createMany({
          data: original.sections.map((section: { type: string; order: number; visible: boolean; content: string }) => ({
