...
import type { SectionType, SectionContent } from "@bettaresume/types";
...
export function useSectionMutations(resumeId: string | null | undefined) {
  ...
  const invalidateResume = () => {
    if (resumeId) {
      ...
      // Don't invalidate resume.list - it's not needed when editing a single resume
  ...
  const reorder = api.section.reorder.useMutation({
    onMutate: async ({ sectionIds }) => {
      ...
      // Optimistically reorder sections - preserve the exact section type from the cache
      if (previousResume) {
  ...
  return {
    ...
    bulkUpsertSections: (sections: Array<{
      id?: string;
      type: SectionType;
      order: number;
      visible: boolean;
    ...
    isAnyPending: create.isPending || update.isPending || remove.isPending || reorder.isPending || bulkUpsert.isPending,
  };
}
