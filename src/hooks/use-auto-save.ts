...
export function useAutoSave<T>({
  ...
  useEffect(() => {
    ...
    if (serverDataStr !== savedDataRef.current && localDataStr === savedDataRef.current) {
      setLocalData(data);
    ...
    try {
      ...
      setTimeout(() => {
        if (isMountedRef.current && status === 'saved') {
          ...
          const currentLocalStr = JSON.stringify(localData);
          if (currentLocalStr === savedDataRef.current) {
            setStatus('idle');
          }
        }
      }, 2000);
