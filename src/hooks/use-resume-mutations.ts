...
import type { CreateResumeInput, UpdateResumeInput } from "@bettaresume/types";
...
export function useResumeMutations() {
  ...
  const update = api.resume.update.useMutation({
    onMutate: async ({ id, data }) => {
      ...
      // Optimistically update single resume - spread preserves the exact type
      if (previousResume) {
    ...
    onSettled: (_data, _error, { id }) => {
      ...
      // The list will be refreshed when navigating back to the dashboard
  ...
  const remove = api.resume.delete.useMutation({
    onMutate: async ({ id }) => {
      ...
      if (previousList) {
        utils.resume.list.setData(undefined, previousList.filter((r) => r.id !== id));
      }
  ...
  const archive = api.resume.archive.useMutation({
    onMutate: async ({ id, archived }) => {
      ...
      // Optimistically update archive status - map preserves the array element type
      if (previousList) {
        utils.resume.list.setData(undefined, previousList.map((r) =>
          r.id === id ? { ...r, isArchived: archived, updatedAt: new Date() } : r
        ));
      }
  ...
  return {
    ...
    updateResume: (id: string, data: UpdateResumeInput) => update.mutateAsync({ id, data }),
    deleteResume: (id: string) => remove.mutateAsync({ id }),
    duplicateResume: (id: string, newName?: string) => duplicate.mutateAsync({ id, newName }),
    archiveResume: (id: string, archived: boolean) => archive.mutateAsync({ id, archived }),
    // Loading states
    ...
    isAnyPending: create.isPending || update.isPending || remove.isPending || duplicate.isPending || archive.isPending,
  };
}
