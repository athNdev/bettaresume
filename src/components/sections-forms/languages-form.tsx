        ...
        <div className="space-y-3">
          {localData.map((lang) => (
            <div key={lang.id} className="flex items-center gap-3 p-3 border rounded-lg">
              ...
              <Button variant="ghost" size="icon" className="h-9 w-9 text-destructive shrink-0" onClick={async () => { const confirmed = await confirm('Remove Language', 'Remove this language?'); if (confirmed) removeLanguage(lang.id); }}>
