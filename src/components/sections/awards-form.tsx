...
import { Plus, Trash2, GripVertical, Trophy, ChevronUp, ChevronDown } from 'lucide-react';
      ...
      {data.length === 0 ? (
        <Card className="border-dashed">
          <CardContent className="flex flex-col items-center justify-center py-8">
            ...
            <p className="text-sm text-muted-foreground mb-4">Showcase your achievements and recognition.</p>
              ...
              <AccordionTrigger className="px-4 py-3 hover:no-underline hover:bg-muted/50">
                <div className="flex items-center gap-3 w-full">
                  ...
                  <div className="flex-1 text-left">
                    ...
                    <div className="text-sm text-muted-foreground">{award.issuer || 'Issuer'}</div>
                  </div>
                  <div className="flex items-center gap-1 mr-2">
                    ...
                    <Button variant="ghost" size="icon" className="h-7 w-7 text-destructive" onClick={(e) => { e.stopPropagation(); if (confirm('Remove this award?')) removeAward(award.id); }}><Trash2 className="h-4 w-4" /></Button>
                  </div>
                </div>
              </AccordionTrigger>
              <AccordionContent className="px-4 pb-4 pt-2">
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    <Input type="month" value={award.date} onChange={(e) => updateAward(award.id, { date: e.target.value })} className="mt-1" />
                  </div>

                  <div>
                    <Label>Description (optional)</Label>
                    <Textarea value={award.description || ''} onChange={(e) => updateAward(award.id, { description: e.target.value })} placeholder="Brief description of the award and why you received it..." className="mt-1" />
                  </div>
                </div>
              </AccordionContent>
            </AccordionItem>
