...
export function Preview({ resume, scale = 1, className }: PreviewProps) {
    ...
    >
      ...
      >
        ...
        {personalInfo.linkedin && (
          <a href={personalInfo.linkedin} style={{ color: colors.accent }}>LinkedIn</a>
        )}
        {personalInfo.github && (
          <a href={personalInfo.github} style={{ color: colors.accent }}>GitHub</a>
        )}
        {personalInfo.website && (
          <a href={personalInfo.website} style={{ color: colors.accent }}>Portfolio</a>
        )}
      </div>
    </div>
  );

  const sectionSpacingPx = settings.sectionSpacing === 'compact' ? 12 : settings.sectionSpacing === 'spacious' ? 24 : 16;
  ...
  const SectionTitle = ({ children }: { children: React.ReactNode }) => (
    <h2
      style={{
        ...
        borderBottom: settings.accentStyle === 'underline' ? `2px solid ${colors.accent}` : 'none',
      }}
    >
      {children}
    </h2>
  ...
  const renderSummary = (section: ResumeSection) => {
    ...
    return (
      <div style={{ marginBottom: sectionSpacingPx * scale }}>
        <SectionTitle>{section.content.title || 'Professional Summary'}</SectionTitle>
        <div
          style={{ fontSize: typography.body * scale, lineHeight: settings.lineHeight * 1.2 }}
  ...
  const renderExperience = (section: ResumeSection) => {
    ...
    return (
      <div style={{ marginBottom: sectionSpacingPx * scale }}>
        <SectionTitle>{section.content.title || 'Work Experience'}</SectionTitle>
        {experiences.map((exp) => (
          <div key={exp.id} style={{ marginBottom: 10 * scale }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
              <div>
                <div style={{ fontSize: typography.itemTitle * scale, fontWeight: 600, color: colors.heading }}>
                  {exp.position}
                </div>
                <div style={{ fontSize: typography.body * scale, color: colors.secondary }}>
                  {exp.company}
                  {exp.location && ` • ${exp.location}`}
                </div>
              </div>
              <div style={{ fontSize: typography.small * scale, color: colors.secondary }}>
                {exp.startDate} - {exp.current ? 'Present' : exp.endDate}
              </div>
            </div>
            {exp.description && (
              <p style={{ fontSize: typography.body * scale, marginTop: 4 * scale }}>{exp.description}</p>
            )}
            {exp.highlights && exp.highlights.length > 0 && (
              <ul style={{ paddingLeft: 16 * scale, marginTop: 4 * scale }}>
                {exp.highlights.map((h, i) => (
                  <li key={i} style={{ fontSize: typography.body * scale, marginBottom: 2 * scale }}>
                    {h}
                  </li>
                ))}
              </ul>
      ...
      <div style={{ marginBottom: sectionSpacingPx * scale }}>
        ...
        {categories.map((cat) => (
          <div key={cat.id} style={{ marginBottom: 8 * scale }}>
            <div style={{ fontSize: typography.itemTitle * scale, fontWeight: 600, color: colors.heading, marginBottom: 4 * scale }}>
              {cat.name}
            </div>
      ...
      <div style={{ marginBottom: sectionSpacingPx * scale }}>
        ...
        {projects.map((proj) => (
          <div key={proj.id} style={{ marginBottom: 10 * scale }}>
            ...
            <p style={{ fontSize: typography.body * scale, marginTop: 4 * scale }}>{proj.description}</p>
            {proj.technologies && proj.technologies.length > 0 && (
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 4 * scale, marginTop: 4 * scale }}>
          ...
          <div key={cert.id} style={{ marginBottom: 8 * scale }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
              <div>
                ...
                <div style={{ fontSize: typography.body * scale, color: colors.secondary }}>{cert.issuer}</div>
              </div>
              <div style={{ fontSize: typography.small * scale, color: colors.secondary }}>{cert.date}</div>
            </div>
          </div>
        ))}
      </div>
