...
import type { Resume, ResumeSection, SyncStatus, SyncQueueItem, SyncState } from '@/types/resume';
...
export class SyncManager {
  ...
  async initialize(): Promise<{ resumes: Resume[]; backendStatus: BackendStatus }> {
    if (this.isInitialized) {
      return { resumes: this.cachedResumes, backendStatus: this.syncState.status === 'offline' ? 'offline' : 'online' };
    }
    ...
    console.log('[SyncManager] Pending sync operations:', this.syncQueue.length);
  ...
  private queueSyncOperation(
    ...
    if (operation !== 'delete' || !this.syncQueue.some(
      o => o.entity === entity && o.entityId === entityId && o.operation === 'create'
    )) {
